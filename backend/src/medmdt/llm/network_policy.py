"""Deterministic network-target policy shared by API and runtime callers.
...
def validate_base_url(provider: str, base_url: str | None) -> None:
    """Validate an effective endpoint target immediately before network use."""
    if not base_url:
...
validate_user_base_url = validate_base_url
